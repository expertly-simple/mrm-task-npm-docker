...
const scripts = [
  ...
  [
    "docker:stop",
    "cross-conf-env docker stop $npm_package_config_imageName || true",
  ],
  ["docker:run", "run-s -c docker:stop docker:runHelper"],
  [
    "docker:runHelper",
    "cross-conf-env docker run -e NODE_ENV=local --rm --name $npm_package_config_imageName -d -p $npm_package_config_imagePort:$npm_package_config_internalContainerPort $npm_package_config_imageRepo",
  ],
  [
    "predocker:publish",
    "echo Attention! Ensure `docker login` information is correct.",
  ],
  [
    "docker:publish",
  ...
  [
    "docker:taillogs",
    "echo Web Server Logs: && cross-conf-env docker logs -f $npm_package_config_imageName",
  ],
  [
    "docker:open",
    "sleep 2 && cross-conf-env open-cli http://localhost:$npm_package_config_imagePort",
  ],
  [
    "docker:debugmessage",
  ...
  [
    "docker:debug",
    "run-s -cs docker:build docker:run docker:open docker:debugmessage docker:taillogs",
  ],
];
